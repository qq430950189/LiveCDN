...
docker/                  Controller Dockerfile（仅核心服务；Agent 不使用 Docker 部署）
origin/                  SRS 配置
web/                     播放器和管理静态页面
...
边缘 Agent 不使用 Docker。Docker 只用于核心 VPS 侧的 Controller / SRS / Web 播放器编排；边缘 NAT 节点使用 `deploy/build-binaries.sh` 产出的 musl 静态二进制，通过 `deploy/install.sh` 写入配置并交给 systemd 管理。仓库不再提供 Agent Dockerfile，测试环境也不再用 Docker service 模拟边缘节点，避免偏离低配 NAT 机器的真实部署路径。
